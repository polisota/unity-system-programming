...
public class PlayerCharacter : Character
{
    ...
    public override void Movement()
    {
        ...
        if (hasAuthority)
        {
            ...
            CmdUpdateRotation(transform.rotation, camera.transform.rotation);
        }
        else
        {
            transform.position = Vector3.SmoothDamp(transform.position, serverPosition, ref currentVelocity, movingSpeed * Time.deltaTime);
            transform.rotation = Quaternion.RotateTowards(transform.rotation, serverRotationY, 1000 * Time.deltaTime);
            camera.transform.rotation = Quaternion.RotateTowards(camera.transform.rotation, serverRotationX, 1000 * Time.deltaTime);
        }
    }
